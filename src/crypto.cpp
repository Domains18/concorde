...
DeviceIdentity DeviceIdentity::load_or_create()
{
    ...
    struct stat buffer;
    bool keys_exist =
        (stat(privkey_path.c_str(), &buffer) == 0) && (stat(pubkey_path.c_str(), &buffer) == 0);
...
{
    std::string hash =
        CryptoUtils::sha256(std::string(keypair_.public_key.begin(), keypair_.public_key.end()));
...
std::string DeviceIdentity::sign(const std::string& data) const
{
    ...
    crypto_sign_detached(signature.data(), nullptr, reinterpret_cast<const uint8_t*>(data.data()),
                         data.size(), keypair_.private_key.data());
...
bool CryptoUtils::verify_signature(const std::string& pubkey_hex, const std::string& data,
                                   const std::string& signature_hex)
{
    ...
    int result =
        crypto_sign_verify_detached(signature.data(), reinterpret_cast<const uint8_t*>(data.data()),
                                    data.size(), pubkey.data());
...
std::string CryptoUtils::sha256(const std::string& data)
{
    ...
    crypto_hash_sha256(hash.data(), reinterpret_cast<const uint8_t*>(data.data()), data.size());
